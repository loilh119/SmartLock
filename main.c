...
static volatile bool scan_int_act = false;
static volatile bool scan_finger_act = false;
static volatile bool delete_finger_act = false;
...
uint8_t sml_unlock[4]     = "ULCK";
uint8_t sml_lock[4]       = "LCKD";

uint8_t sml_dele[4]       = "DESU";
uint8_t sml_dele_err[4]   = "DEER";

uint8_t sml_finger[4]     = "FISU";
uint8_t sml_finger_err[4] = "FIER";

uint8_t template_id_send[3] = {0x00,0x00,0x0D};
 ...
 */
static ble_uuid_t m_adv_uuids[] = {{SERVICE_UUID, BLE_UUID_TYPE_VENDOR_BEGIN}};
...
/**@brief Function for handling events from button 3, open door.
 *
 * @param[in] 
 */
static void open_door_button()
{
		ret_code_t err_code;
	...
	//		ble_sml_custom_value_update(&m_sml, sml_unlock, 4);
...
uint8_t get_data(ble_sml_evt_t * p_evt)
{
		memset(data_send,0,20);
		for (int i = 0; i < p_evt->params.sml_data.length; i++)
		{
				data_send[i] = p_evt->params.sml_data.p_data[i];
		}
		data_send[p_evt->params.sml_data.length] = 0x0D;
		return p_evt->params.sml_data.length + 1;
}

void send_alarm(uint8_t length)
{
		nrf_gpio_pin_toggle(LED_2);
		ble_alarm_c_string_send(&m_alarm_c, data_send, length);
		ble_alarm_c_string_send(&m_alarm_c, template_id_send,3);
}
			...
			else if(check == 2)
			{
				switch(check_sum(4,12))
				{
					...
					default:
						...
						ble_sml_custom_value_update(&m_sml, sml_finger_err, 4);
...
//						
//						sub_data[0] = 0x01;
//						sub_data[2] = Template_ID >> 8;
//						sub_data[3] = Template_ID;
			...
			else if(check == 3)
			{
				switch(check_sum(4,12))
				{
					case 0x00:
						...
						ble_sml_custom_value_update(&m_sml, sml_finger, 4);		
						
						template_id_send[0] = Template_ID >> 8;
						template_id_send[1] = Template_ID;
						send_alarm(data_length);
			...
			else if(check == 1)
			{
				switch(check_sum(4,12))
				{
					case 0x00:
						...
						ble_sml_custom_value_update(&m_sml, sml_dele, 4);
					...
					default:
						...
						ble_sml_custom_value_update(&m_sml, sml_dele_err, 4);
...
static void delay_handler(void * p_context)
{
    ...
    UNUSED_PARAMETER(p_context);
		...
		{
			ble_sml_custom_value_update(&m_sml, sml_lock, 4);
...
static void on_sml_evt(ble_sml_t * p_sml_service, ble_sml_evt_t * p_evt)
{
		ret_code_t err_code;
            ...
            break;
				...
				case BLE_SML_EVT_FINGER:
					if(scan_finger_act == false)
					{
						...
						Template_ID = (p_evt->params.sml_data.p_data[1] << 8) + p_evt->params.sml_data.p_data[2];
...
void sensor(nrf_drv_gpiote_pin_t pin, nrf_gpiote_polarity_t action)
{
	...
	{
		if(m_sml.lock_status == LOCK_OPEN)
		{
	...
	else if(nrf_gpio_pin_read(pin) == 0)
	{
		if(m_sml.lock_status == LOCK_CLOSE)
		{
			data_send[0] = 's';
			data_send[1] = 0x0D;
			template_id_send[0] = 0x00;
...
static void gpio_init(void)
{
    ...
    nrf_drv_gpiote_in_config_t in_config_finger = NRFX_GPIOTE_CONFIG_IN_SENSE_HITOLO(true);
    //in_config_finger.pull = NRF_GPIO_PIN_PULLDOWN;

    err_code = nrf_drv_gpiote_in_init(4, &in_config_finger, finger_scan);
    APP_ERROR_CHECK(err_code);
		
    nrf_drv_gpiote_in_event_enable(4, true);
		
		nrf_drv_gpiote_in_config_t in_config_sensor = NRFX_GPIOTE_CONFIG_IN_SENSE_TOGGLE(true);
    in_config_sensor.pull = NRF_GPIO_PIN_PULLDOWN;
		
		err_code = nrf_drv_gpiote_in_init(29, &in_config_sensor, sensor);
    APP_ERROR_CHECK(err_code);
